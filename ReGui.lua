...
return end end self:Remove()self.IsClosed=true end function p:SetVisible(q)local
r,s=self.WindowFrame,self.NoFocusOnAppearing self.Visible=q r.Visible=q if q and
not s then aa:SetFocusedWindow(self)end return self end function p:
ToggleVisibility(q)local r=self.Visible self:SetVisible(not r)end function p:
GetWindowSize()return self.WindowFrame.AbsoluteSize end function p:
GetTitleBarSizeY()local q=self.TitleBar return q.Visible and q.AbsoluteSize.Y or
0 end function p:SetTitle(q)self.TitleLabel.Text=tostring(q)return self end
function p:SetPosition(q)self.WindowFrame.Position=q return self end function p:
SetSize(q,r)local s=self.WindowFrame if typeof(q)=='Vector2'then q=UDim2.
fromOffset(q.X,q.Y)end s.Size=q self.Size=q return self end function p:
SetCanvasInteractable(q)local r=self.Body r.Interactable=q end function p:Center
()local q=self:GetWindowSize()/2 local r=UDim2.new(0.5,-q.X,0.5,-q.Y)self:
SetPosition(r)return self end function p:LoadStylesIntoElement(q)local r,s,t=q.
Flags,q.Object,q.Canvas local u={FrameRounding=function()if r.CornerRadius then
return end if not t then return end local u=s:FindFirstChild('FrameRounding',
true)if not u then return end t:TagElements{[u]='FrameRounding'}end}for v,x in u
do local y=self:GetThemeKey(v)if y==nil then continue end task.spawn(x,y)end end
function p:SetTheme(q)local r,s,t=aa.ThemeConfigs,self.TagsList,self.WindowState
q=q or self.Theme assert(r[q],`{q} is not a valid theme!`)self.Theme=q aa:
MultiUpdateColors{Animate=false,Theme=q,Objects=s}self:SetFocusedColors(t)return
self end function p:SetFocusedColors(q)local r,s,t,u,v=self.WindowFrame,self.
TitleBar,self.Theme,self.TitleLabel,self:GetThemeKey'AnimationTweenInfo'local x=
r:FindFirstChildOfClass'UIStroke'local y={Focused={[x]='BorderActive',[s]=
